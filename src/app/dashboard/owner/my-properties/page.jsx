...
const MyProperties = () => {
  ...
  const fetchProperties = () => {
    ...
    axiosSecure.get(`/properties/owner/${user.email}`)
      ...
      .catch(err => console.log('Error fetching properties:', err.message))
  ...
  return (
    <div>
      <span className="font-mono text-xs uppercase tracking-widest text-clay">— Your listings</span>
      <h1 className="font-display italic text-3xl text-ink mt-1 mb-8">My Properties</h1>

      {properties.length === 0 ? (
        <div className="border border-dashed border-line rounded-sm py-16 text-center">
          <p className="text-muted text-sm">You haven&apos;t listed any properties yet.</p>
        </div>
      ) : (
                ...
                <tr key={property._id} className="border-t border-line">
                  ...
                  <td className="px-4 py-3">
                    ...
                    {property.status === 'rejected' && property.rejectionFeedback && (
                      <button
                        onClick={() => setFeedbackModal(property.rejectionFeedback)}
      ...
      {feedbackModal && (
        <div className="fixed inset-0 bg-ink/50 flex items-center justify-center z-50 px-6">
          <div className="bg-paper rounded-sm p-6 w-full max-w-md">
            ...
            <p className="text-sm text-ink/80 leading-relaxed">{feedbackModal}</p>
            <button
