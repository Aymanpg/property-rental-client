...
const TagMark = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="-rotate-12">
    <path d="M3 11.5L11.5 3H19a2 2 0 0 1 2 2v7.5L12.5 21 3 11.5Z" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round" />
...
const Navbar = () => {
  ...
  const dashboardPath =
    dbUser?.role === 'admin' ? '/dashboard/admin/users' :
    dbUser?.role === 'owner' ? '/dashboard/owner' :
    ...
    <header className="sticky top-0 z-50 bg-paper/95 backdrop-blur border-b-2 border-ink">
      <nav className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2.5 text-ink group">
          <span className="text-clay group-hover:rotate-0 transition-transform"><TagMark /></span>
          <span className="font-display italic text-2xl tracking-tight">RentEase</span>
        </Link>
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-56 bg-paper border border-line rounded-sm shadow-lg z-20 overflow-hidden">
                    <div className="px-4 py-3 border-b border-line">
                      <p className="text-sm font-medium text-ink truncate">{dbUser?.name || user?.displayName}</p>
                      <p className="text-xs text-muted truncate">{dbUser?.email || user?.email}</p>
      ...
      {menuOpen && (
        <div className="md:hidden border-t border-line px-6 py-5 flex flex-col gap-4 bg-paper">
          ...
          <Link href="/properties" className={linkClass('/properties')} onClick={() => setMenuOpen(false)}>All Properties</Link>

          {!user ? (
            <>
              <Link href="/login" className={linkClass('/login')} onClick={() => setMenuOpen(false)}>Login</Link>
              <Link
                href="/register"
                onClick={() => setMenuOpen(false)}
                className="inline-flex w-fit px-5 py-2.5 bg-ink text-paper text-sm font-semibold rounded-sm"
              >
                Get started
              </Link>
            </>
            ...
            <>
              <div className="flex items-center gap-3 py-2">
                ...
                <div>
                  <p className="text-sm font-medium text-ink">{dbUser?.name || user?.displayName}</p>
                  <p className="text-xs text-muted">{dbUser?.role || 'tenant'}</p>
                </div>
              </div>
              <Link href={dashboardPath} className={linkClass(dashboardPath)} onClick={() => setMenuOpen(false)}>Dashboard</Link>
              <button onClick={handleLogout} className="text-left text-sm font-semibold text-clay">
                Logout
              </button>
            </>
          )}
